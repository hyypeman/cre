...
def parse_result(result_row):
    ...
    if "previously" in full_result_text.lower() or "alternatively known as" in full_result_text.lower():
        # Check if our search term is mentioned in the alternative names
        alt_name_match = re.search(r'(?:previously|alternatively)\s+known\s+as\s+(.*?)(?:\)|$)', full_result_text, re.IGNORECASE)
        if alt_name_match:
...
def normalize_company_name(name):
    ...
    legal_suffixes = [
        r'\bllc\b', r'\binc\b', r'\binc\.$', r'\bltd\b', r'\bltd\.$', 
        r'\bcorp\b', r'\bcorp\.$', r'\bcorporation\b', r'\bco\b', r'\bco\.$',
        r'\bcompany\b', r'\blimited\b', r'\blp\b', r'\bl\.p\.$',
        r'\bplc\b', r'\bp\.l\.c\.$', r'\bllp\b', r'\bl\.l\.p\.$',
    ]
...
def extract_company_info(page: Page) -> Dict[str, Any]:
    ...
    if branch_elem.count() > 0:
        ...
        if branch_company_link.count() > 0:
            ...
            branch_company_url = 'https://opencorporates.com' + branch_company_link.get_attribute('href')
            ...
            branch_jurisdiction = jurisdiction_match.group(1).strip() if jurisdiction_match else None
    ...
    contact_person = None
    registered_address_lines = page.locator('dd.registered_address ul.address_lines li.address_line')
    ...
    attributes['officers'] = _extract_officers(page, 'dd.officers ul.attribute_list li.attribute_item')
    
    # Extract inactive officers/directors
    attributes['inactive_officers'] = _extract_officers(page, 'dd.inactive_officers ul.attribute_list li.attribute_item', inactive=True)
    ...
    home_company_row = page.locator('#data-table-branch_relationship_object table.company-data-object tbody tr')
    if home_company_row.count() > 0:
        ...
        if home_company_link.count() > 0:
            ...
            home_company_url = 'https://opencorporates.com' + home_company_link.get_attribute('href')
            
            # Extract jurisdiction
            jurisdiction_link = home_company_row.locator('a.jurisdiction_filter')
            jurisdiction = jurisdiction_link.inner_text().strip() if jurisdiction_link.count() > 0 else None
            ...
            details_url = 'https://opencorporates.com' + details_link.get_attribute('href') if details_link.count() > 0 else None
    ...
    with sync_playwright() as p:
        ...
        try:            
            ...
            for matching_record in matching_records:
                ...
                if page.locator('//dd[contains(@class, "officers")] /a[contains(text(), "please log in")]').count() > 0:
...
if __name__ == '__main__':
    search_opencorporate('44 West 34th Street')